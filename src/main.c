...
static void last5_window_load(Window *window) {
  ...
  int rectBounds = 0;
  for(int i=0; i<5; i++){
    snprintf(day[i], sizeof(day[i]),"Mood: %d, Sleep (h): %d, Steps: %d", a[i].mood, (a[i].sleepSeconds+1800)/3600, a[i].steps);
...
static void graph_window_load(Window *window) {
 ...
 white_bar = text_layer_create(GRect(0,0, bounds.size.w, 30));
